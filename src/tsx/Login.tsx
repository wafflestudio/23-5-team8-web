...
export default function Login() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (!userData || !('id' in userData) || !('nickname' in userData)) {
        console.error('[Login] Unexpected login response format:', responseData);
        setError('root', {message: '로그인 중 알 수 없는 오류가 발생했습니다.'});
        return;
      }
        ...
        if (isAxiosError(error) && error.response) {
          ...
          switch (status) {
            ...
            case 401:
              ...
              break;
            default:
              setSocialErrorMessage('소셜 로그인 중 알 수 없는 오류가 발생했습니다.');
              console.error('[Login] Social Login Failed:', error.response.data);
          }
        } else {
          console.error('[Login] Unexpected Error:', error);
  ...
  useEffect(() => {
    ...
    const provider = sessionStorage.getItem('pendingSocialProvider') as socialProvider | null;
      ...
      <main className='login-main'>
        <div className='login-container'>
          ...
          <div className='social-login'>
            ...
            <button className='kakao-login-button' onClick={() => handleSocialStart('kakao')}>
              <div className='kakao-icon-wrapper'>
                <img src='/assets/kakao_logo.png' alt='Kakao Symbol' />
              </div>
              <span className='kakao-label'>카카오 로그인</span>
            </button>
            <button className='gsi-material-button' onClick={() => handleSocialStart('google')}>
